  ...
  >
    <v-card class="card px-2">
      ...
      <div class="scroll-content pb-3">
        <div v-if="!searchReady" class="mt-3">
        ...
        <div v-else class="mt-3">
          ...
          <div v-if="tagSearchResult.length" class="body-2">検索結果</div>
...
export default {
  ...
  computed: {
    position() {
      return {
        x:
          window.innerWidth -
          this.$store.state.settings.renderer.app.editMenuWidth,
  ...
  methods: {
    async show(contentIDs) {
      this.input = ""
      this.contentIDs = Array.isArray(contentIDs) ? contentIDs : [contentIDs]
      this.visible = true
    ...
    async toggleTag(targetTag) {
      ...
      if (
        this.selectedTags.find(
          (selectedTag) => selectedTag.tagID === targetTag.tagID
        )
      ) {
