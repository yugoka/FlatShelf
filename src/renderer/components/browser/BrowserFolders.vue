  ...
  >
    <v-expansion-panel>
      ...
      <v-expansion-panel-content>
        <v-row class="folders-wrapper" no-gutters>
          <v-col
...
export default {
  ...
  data() {
    return {
      ...
      includeDecendantFolders:
        this.$config.renderer().search.query.includeDecendantFolders,
    }
  },
  computed: {
    currentFolderID() {
  ...
  watch: {
    ...
    includeDecendantFolders() {
      this.$config.set(
        "renderer.search.query.includeDecendantFolders",
        this.includeDecendantFolders
      )
    },
  },
  methods: {
    async getChildrenFolders() {
      this.childrenFolders = await this.$folders.getDecendants(
